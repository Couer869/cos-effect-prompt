...
description: >
  ...
  用户提到「3D素材 / 道具模型 / 置入模型 / 素材太假 / 武器融合 / 悬浮感」「面部精修 /
  自然感修容 / 厚涂修容 / CG感修容」「重塑光影 / 打光 / 布光 / 逆光 / 侧光 / 色温 / 电影光」
  「服装材质 / 缎面 / 雪纺 / 皮革 / 蕾丝 / 金属 / C服还原」或「机械 / 藤蔓 / 全息 / 凤凰 /
  组合特效 / 打包」时，也务必使用本技能——知识库含 3D 素材整合、光影重塑、服装材质库、
  修容风格变体与 44 种特效 + 组合模板。
  用户提到「瑕疵移除 / 祛痘 / 祛疤痕 / 体毛 / 肩颈优化 / 抹油 / 绷紧衣服 / 胸部扩大 /
  腰部纤细 / 腹肌线条 / 刘海修整 / CG毛发 / 亮闪闪头发 / 长发飘动」等**身体与毛发专项**
  需求，或提到「王家卫 / 银翼杀手 / 波顿 / 复古胶片 / 黑白片 / 蒸汽朋克 / 新海诚 / 宫崎骏 /
  电影风格 / 仿XX电影」等**电影风格**需求，或提到「ControlNet / 控制网 / LoRA / embedding /
  姿势控制 / 深度图」等 **ComfyUI 进阶**需求时，也务必使用本技能——知识库含身体综合修图、
  电影风格仿制、ComfyUI 深度写法（ControlNet/LoRA）与头发专业措辞。
---
...
| 瘦身 / 塑形 / 液化 / 身材 / 胸部 / 腰部 / 腹肌 / 肩颈 / 瑕疵移除 / 祛痘 / 祛疤痕 / 体毛 / 抹油… | `body_sculpting` |
...
- 服装材质：用户提具体材质（缎面/雪纺/皮革/蕾丝/金属/绒面/纱/PVC…）或 C 服还原时，从 `references/clothing-materials.md` 取材质措辞与还原度检查清单（版型/配色/花纹/五金件/标志）
...
> **电影风格仿制**：用户提到「王家卫 / 银翼杀手 / 波顿 / 复古胶片 / 黑白片 / 蒸汽朋克 / 新海诚 / 宫崎骏 / 仿 XX 电影」时，从 `references/cinema-styles.md` 取风格措辞，注入调色（`color_grading`）/ 光影（`lighting_reshape`）/ 质感（`film_grain` 等）。
...
用户把**喜欢的参考提示词**存在 `references/user-prompts.md` 里（可直接手动贴，也可让本技能写入）。作用是：存入时**泛化分析**提炼可复用要素，生成时**参考优化**输出。

### 存入参考（用户提供提示词）

触发：用户粘贴提示词并说"存到参考库 / 保存这个 / 我喜欢这个"。

1. **判定来源模型**：看格式——`--参数` → MJ；逗号 tag / 权重 `(x:1.2)` → SD/ComfyUI；纯标签 → NovelAI；自然语言 → Nano Banana / MJ
2. **泛化分析**：提炼结构（如 [主体]+[场景]+[光线色调]+[质感]+[参数]）、亮点、可迁移写法
3. **写入** `references/user-prompts.md`：按条目模板（参考名 / 来源模型 / 用途标签 / 原文 / 泛化分析 / 可复用要素），**征得用户确认后**追加
4. 告知已存入，可随时用"参考库里的 XX"调用

### 生成时参考（优化输出）

生成提示词前，检查 `references/user-prompts.md`：
- 用户明确说"用参考库的 XX / 按之前存的那个" → 直接读对应条目
- 当前需求与库里某条目的**用途标签匹配** → 参考该条目的结构与措辞
- 参考后**按目标模型格式适配**（Nano Banana 信封 / ComfyUI tag / MJ 自然语言），不照抄原格式；借鉴的是结构与可复用要素，注入的是当前需求

### 泛化分析模式（批量总结）

触发：用户说"分析参考库 / 总结这些提示词 / 找出共性"。

读取 `references/user-prompts.md` 全部条目，输出**泛化总结**：
- 高频结构 / 常用要素 / 风格倾向
- 对当前需求的适用建议
- 可新增到风格库或词库的沉淀

**注意事项**：参考库写入同样是**修改技能文件**的操作——只在用户明确要求存入时执行，写入前展示条目让用户确认。

## 画笔引导（Brush Guidance）

用户在照片上**用红色画笔**做了标记，作为方向/轨迹/区域的**引导基准**。生成头发、服装、吹风等**方向性效果**时，提示词要引用这些标记。

**标记类型与含义**：

| 红笔标记 | 含义 |
|---|---|
| 箭头 | 方向（风的方向、发丝飘向、裙摆/披风扬起方向、特效飞行方向） |
| 轨迹 / 笔迹 | 运动路径或作用范围 |
| 圈选 | 指定作用区域（只在这块改） |

**触发**：用户说"我画了箭头 / 红色画笔标了 / 按我画的方向 / 照着红笔标记 / 用画笔标了风向"。

**注入方式**：在对应模块加 `brush_guidance` 字段：

```json
"brush_guidance": "以照片中红色画笔标记的箭头方向为基准，{目标}沿该方向{动作}；红笔标记是方向指示，不得残留在成片中"
```

**各模块应用**：
- `wind_effect` → 风向以红笔箭头为准（`direction` 写"沿红色箭头方向"）
- `hair_enhancement` → 发丝流向 / 飘动沿红笔方向
- `clothing_refresh` → 裙摆 / 披风 / 飘带的飘动方向以红笔箭头为准
- `special_effects` → 有方向的特效（火焰拖尾 / 风刃 / 光流）可用红笔指定方向

**模型差异**：Nano Banana 能看到照片上的红笔标记 → **直接引用**；ComfyUI / Midjourney / NovelAI 通常看不到标注图 → 把方向**翻译成文字 / tag** 描述（用户口述方向即可，具体写法见 `references/models.md` 画笔引导章节）。

**注意事项**：
- 明确红笔标记是**方向指示**，不是要画进成品的线条，成片不得残留
- 用户说"画笔引导"但照片上没看到红笔标记 → 先确认用户是否已画好，或问标记内容
- 透视 / 光影一致性规则照常适用，画笔只影响方向不破坏其他约束
...
- 用户参考提示词库（存参考 / 泛化分析 / 生成时参考）→ `references/user-prompts.md`
